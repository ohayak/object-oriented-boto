...
def test_sql_handler():
    ...
    os.environ['SECRET_CREDENTIALS_ARN'] = "arn:aws:secretsmanager:eu-west-1:123456789012:secret:rds-db-credentials/master-Hc2JJi"
    sm = client('secretsmanager')
    sm.create_secret(
        Name=os.environ['SECRET_CREDENTIALS_ARN'],
        SecretString='[{"Username":"bob"},{"Password":"abc123"}]',
    )
