...
class AuroraDataAPIClient:
    def __init__(self, dbname=None, aurora_cluster_arn=None, secret_arn=None, rds_data_client=None):
        ...
        if rds_data_client is None:
            self._client = boto_client("rds-data")
        self._dbname = dbname
...
class AuroraDataAPICursor:
    ...
    def _set_description(self, column_metadata):
        ...
        for column in column_metadata:
            col_desc = ColumnDescription(name=column["name"], type_code=self._pg_type_map.get(column["typeName"], str))
    ...
    def _format_parameter_set(self, parameters):
        return [{"name": k, "value": self.prepare_param_value(v)} for k, v in parameters.items()]
    ...
    def executemany(self, operation, seq_of_parameters, batch_size=1000):
        logger.debug("executemany %s", reprlib.repr(operation.strip()))
        for batch in self._page_input(seq_of_parameters, page_size=batch_size):
