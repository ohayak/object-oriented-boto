...
class MySQLConnection(Connection, Extentions):
    ...
    def load_into_s3(self, bucket, key, tbl_name, fields_delimiter=";", lines_delimiter="\n", encoding="utf8"):
        with self.cursor() as cur:
            cur.execute(
                self.format_s3_unload_statement(
                    bucket, key, tbl_name, fields_delimiter=";", lines_delimiter="\n", encoding="utf8"
                )
            )
        self.commit()
    ...
    def get_create_table_query(self, tbl_name):
        with self.cursor() as cur:
...
class PgSQLConnection(connection, Extentions):
    @staticmethod
    def format_s3_load_statement(s3object: S3Object, tbl_name, region, delimiter=";"):
    ...
    def get_create_table_query(self, schema_name, tbl_name):
        with self.cursor() as cur:
